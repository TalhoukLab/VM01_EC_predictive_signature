...
beta_dist <- vegdist((vm.PLSDA_batch), index = "aitchison", na.rm = TRUE)
beta_dist[!is.finite(beta_dist)] <- NA

ps_ord <- ordinate(new_phylo, method = "NMDS", distance = "euclidean")
...
plot_ordination(new_phylo, ps_ord, type = "samples", color = var, 
              ...
              scale_colour_manual(values=c("#32CB46", "#FF3333"))
dev.off()

##Univariate 
test <- adonis2((beta_dist) ~ vm.trt, permutations = 100000, na.action = "na.omit", )
test
