...
for (cohort in cohorts) {
  print(cohort)
  raw_table <- read_biom(file.path(paste0('../VM01_reproducibility_replicability/Tsementzi_pipeline/results/', cohort, '/filtered_sortedUnique_otu.biom')))
  ...
  # read in tax data 
  tax_table <- read.delim(file.path(paste0('../VM01_reproducibility_replicability/Tsementzi_pipeline/results/', cohort, '/seqs_sorted_rep_set_tax_assignments.txt')), header = FALSE, sep = "\t")
  ...
  phylo_tree <- read_tree(file.path(paste0('../VM01_reproducibility_replicability/Tsementzi_pipeline/results/', cohort, '/rep_set.tre')))
