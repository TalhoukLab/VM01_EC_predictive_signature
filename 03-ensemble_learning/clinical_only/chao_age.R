...
test_data <- data.frame(sample_data(Antonio1_solo_res$raw))
test_data_subset <- test_data %>% select(c(age))
rownames(test_data_subset) <- rownames(test_data)

training_data <- data.frame(sample_data(Chao_solo_res$raw))
training_data_subset <- training_data %>% select(c(age, histology))
rownames(training_data_subset) <- rownames(training_data)
training_data_subset$histology <- factor(training_data_subset$histology, levels = c("Benign", "EC"))

neigh <- ifelse(min(table(training_data_subset$histology))>25, 5, min(table(training_data_subset$histology))-1)
training_data_subset$histology <- as.factor(training_data_subset$histology)
training_data_subset <- janitor::clean_names(training_data_subset)
training_data_subset_smote <- themis::smotenc(training_data_subset, var = "histology", k = neigh, over_ratio = 1)

set.seed(123)
folds <- rsample::vfold_cv(training_data_subset_smote, strata = "histology", v = 5, repeats = 3)
## remove correlation filter for base models 
data_rec <- recipe(histology ~ ., data = training_data_subset_smote) %>%
  step_normalize(age)
...
rf_grid <- expand.grid(mtry = 1, min_n = seq(2, 10, 1), trees = seq(1500, 2500, 100))
...
final_fit_rf <- final_rf_wf %>%
  fit(training_data_subset_smote)
rf_results_chao_grid <- get_metrics_indi(data = test_data_subset, model = final_fit_rf, 
                                         level = "genus", labels = test_data$histology)
saveRDS(rand_forest_res_grid, "~/Desktop/thesis/VM02_meta_analysis/src/updated/updated1/Chao_age_rf_grid.rds")
saveRDS(final_fit_rf, "~/Desktop/thesis/VM02_meta_analysis/src/updated/updated1/Chao_age_rf_fit.rds")
...
final_fit_nnet <- final_nnet_wf %>%
  fit(training_data_subset_smote) 
nnet_results_chao_grid <- get_metrics_indi(data = test_data_subset, model = final_fit_nnet, 
                                         level = "genus", labels = test_data$histology)
saveRDS(nnet_res, "~/Desktop/thesis/VM02_meta_analysis/src/updated/updated1/Chao_age_nnet_grid.rds")
saveRDS(final_fit_nnet, "~/Desktop/thesis/VM02_meta_analysis/src/updated/updated1/Chao_age_nnet_fit.rds")
...
xgb_grid <- expand.grid(mtry = 1, trees = 1500, min_n = seq(2, 10, 1), learn_rate = 0.3, 
...
final_fit_bt <- final_bt_wf %>%
  fit(training_data_subset_smote) 
xgb_results_chao_grid <- get_metrics_indi(data = test_data_subset, model = final_fit_bt, 
                                           level = "genus", labels = test_data$histology)
saveRDS(xgb_res, "~/Desktop/thesis/VM02_meta_analysis/src/updated/updated1/Chao_age_xgb_grid.rds")
saveRDS(final_fit_bt, "~/Desktop/thesis/VM02_meta_analysis/src/updated/updated1/Chao_age_xgb_nnet_fit.rds")
