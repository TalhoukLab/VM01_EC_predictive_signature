...
training_data_complete_sub <- training_data_complete %>% select(c(age, bmi, pHRecoded, ethnicity))
training_data_complete_sub$histology <- factor(sample_data(Tsementzi_solo_res$raw)$histology, levels = c("Benign", "EC"))
...
saveRDS(rand_forest_res_grid, "~/Desktop/thesis/VM02_meta_analysis/src/updated/updated1/Tsementzi_all_clinical_rf_grid.rds")
saveRDS(final_fit_rf, "~/Desktop/thesis/VM02_meta_analysis/src/updated/updated1/Tsementzi_all_clinical_rf_fit.rds")
...
nnet_grid <- expand.grid(hidden_units = seq(1, 30, 5), penalty = seq(0, 1, 0.1), epochs = seq(5, 11, 2))
...
nnet_results_chao_grid <- get_metrics_indi(data = test_data_complete_sub, model = final_fit_nnet, 
                                         level = "genus", labels = sample_data(Antonio1_solo_res$raw)$histology)
saveRDS(nnet_res, "~/Desktop/thesis/VM02_meta_analysis/src/updated/updated1/Tsementzi_all_clinical_nnet_grid.rds")
saveRDS(final_fit_nnet, "~/Desktop/thesis/VM02_meta_analysis/src/updated/updated1/Tsementzi_all_clinical_nnet_fit.rds")
...
xgb_results_chao_grid <- get_metrics_indi(data = test_data_complete_sub, model = final_fit_bt, 
                                           level = "genus", labels = sample_data(Antonio1_solo_res$raw)$histology)
saveRDS(xgb_res, "~/Desktop/thesis/VM02_meta_analysis/src/updated/updated1/Tsementzi_all_clinical_xgb_grid.rds")
saveRDS(final_fit_bt, "~/Desktop/thesis/VM02_meta_analysis/src/updated/updated1/Tsementzi_all_clinical_xgb_fit.rds")
