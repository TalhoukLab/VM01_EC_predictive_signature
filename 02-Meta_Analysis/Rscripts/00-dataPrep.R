...
cohorts <- c("all_cohorts", "AWT")
for(cohort in cohorts){
  raw_otus <- read.delim(file.path(paste0('../vaginalMicrobiome/02-Meta_Analysis/Raw/', 
                                          cohort, '/all.otutab.txt')))
  ...
  rep_seqs <- readDNAStringSet(file.path(paste0('../vaginalMicrobiome/02-Meta_Analysis/Raw/', 
                                                cohort, '/all.otus.fasta')))
  ...
  feature_table <- read.delim(file.path(paste0('../vaginalMicrobiome/02-Meta_Analysis/00-helperfiles/', 
                                               cohort, 'FT.csv')), header = TRUE, sep = ",")
  ...
  #pred_copy1 <- pred
  phylo_tree <- read_tree(file.path(paste0('../vaginalMicrobiome/02-Meta_Analysis/Raw/', 
                                           cohort, '/otus.tree')))
