...
prep_test_age_bmi_eth <- function(data, trained_data){
  ...
  if("shannon" %in% colnames(otu_test)){
    otu_test <- subset(otu_test, select = -c(shannon, labels, age, bmi, ethnicity) )
  } else {
    otu_test <- otu_test
  }
  otu_test <- otu_test + 1
...
prep_test_age_bmi_eth_wal <- function(data, trained_data){
  data <- data.frame(janitor::clean_names(data))
  trained_data <- data.frame(janitor::clean_names(trained_data))
  otu_test <- fncols_antonio(data, setdiff(colnames(trained_data), colnames(data)))
  otu_test <- base::subset(otu_test, select = colnames(trained_data))
...
get_metrics_indi <- function(data, model, level, labels, cohort){
  ...
  if(cohort == "tsementzi"){
    preds <- prep_test_age_bmi_eth(data = data, trained_data = model$pre$actions$recipe$recipe$template) %>%
      bind_cols(predict(model, ., type = "prob"))%>%
      bind_cols("labels"=labels)
  }
  if(cohort == "walsh"){
    preds <- prep_test_age_bmi_eth_wal(data = data, trained_data = model$pre$actions$recipe$recipe$template) %>%
      bind_cols(predict(model, ., type = "prob"))%>%
      bind_cols("labels"=labels)
  }
...
## Tsementzi 
tsementzi_rf_grid <- readRDS("~/Desktop/thesis/VM02_meta_analysis/src/updated/Tsementzi2_clinical_no_ph_micro_rf_grid.rds")
tsementzi_rf_fit <- readRDS("~/Desktop/thesis/VM02_meta_analysis/src/updated/Tsementzi3_clinical_no_ph_micro_rf_fit.rds")
tsementzi_nnet_grid <- readRDS("~/Desktop/thesis/VM02_meta_analysis/src/updated/Tsementzi1_clinical_no_ph_micro_nnet_grid.rds")
tsementzi_nnet_fit <- readRDS("~/Desktop/thesis/VM02_meta_analysis/src/updated/Tsementzi1_clinical_no_ph_micro_nnet_fit.rds")
tsementzi_xgb_grid <- readRDS("~/Desktop/thesis/VM02_meta_analysis/src/updated/Tsementzi1_clinical_no_ph_micro_xgb_grid.rds")
tsementzi_xgb_fit <- readRDS("~/Desktop/thesis/VM02_meta_analysis/src/updated/Tsementzi1_clinical_no_ph_micro_xgb_fit.rds")

## Walsh
#walsh_rf_grid <- readRDS("~/Desktop/thesis/VM02_meta_analysis/src/updated/updated1/Walsh_clinical_no_phmicro_rf_grid.rds")
walsh_rf_fit <- readRDS("~/Desktop/thesis/VM02_meta_analysis/src/updated/updated1/Walsh1_clinical_no_phmicro_rf_fit.rds")
...
walsh_xgb_fit <- readRDS("~/Desktop/thesis/VM02_meta_analysis/src/updated/updated1/Walsh1_clinical_no_ph_micro_xgb_fit.rds")
...
get_results_microbiome_walsh <- function(cohort, type){
  if(type == "training"){
    rf_results <- get_metrics_indi_tr(data = eval(parse(text = paste0(cohort, "_rf_fit")))$pre$actions$recipe$recipe$template, 
                                      model = eval(parse(text = paste0(cohort, "_rf_fit"))), 
                                      level = "genus", labels = eval(parse(text = paste0(cohort, "_rf_fit")))$pre$mold$outcomes$labels, cohort = cohort)
  ...
  xgb_pred <- xgb_results$predictions %>%
    dplyr::select(c(.pred_EC)) %>%
    dplyr::rename(!!paste0(cohort, "_xgb") := .pred_EC)
  combined <- cbind(rf_pred, xgb_pred)
  if(type == "training"){
    combined <- combined %>% 
      bind_cols("labels" = factor(eval(parse(text = paste0(cohort, "_rf_fit")))$pre$mold$outcomes$labels, levels = c("Benign", "EC")))
    combined$ensemble <- rowSums(combined[,c(1:2)])/2
  } else {
    combined <- combined %>%
      bind_cols("labels" = factor(sample_data(Antonio1_solo_res$raw)$histology, levels = c("Benign", "EC")),
                "sraID" = sample_data(Antonio1_solo_res$raw)$sraID)
    combined$ensemble <- rowSums(combined[,c(1:2)])/2
    combined <- combined %>% relocate(sraID, .after = ensemble)
    
  }
  combined <- combined %>% relocate(labels, .after = ensemble) %>%
    dplyr::rename(!!paste0(cohort, "_ensemble") := ensemble)
  all_metrics <- lapply(combined[, c(1:3)], get_metrics, data = combined)
...
get_results_microbiome_tsementzi1 <- function(cohort, type){
  if(type == "training"){
    rf_results <- get_metrics_indi_tr(data = eval(parse(text = paste0(cohort, "_rf_fit")))$pre$actions$recipe$recipe$template, 
                                      model = eval(parse(text = paste0(cohort, "_rf_fit"))), 
                                      level = "genus", labels = eval(parse(text = paste0(cohort, "_rf_fit")))$pre$mold$outcomes$labels, cohort = cohort)
    ...
    nnet_results <- get_metrics_indi_tr(data = eval(parse(text = paste0(cohort, "_rf_fit")))$pre$mold$predictors, 
                                       model = eval(parse(text = paste0(cohort, "_nnet_fit"))), 
                                       level = "genus", labels = eval(parse(text = paste0(cohort, "_nnet_fit")))$pre$mold$outcomes$labels, cohort = cohort)
    nnet_results$met$model <- "nnet"
  }
  if(type == "testing"){
    rf_results <- get_metrics_indi(data = test_data, model = eval(parse(text = paste0(cohort, "_rf_fit"))), level = "genus", labels = sample_data(Antonio1_solo_res$raw)$histology, cohort = cohort)
...
get_results_microbiome_tsementzi <- function(cohort, type){
  if(type == "training"){
    rf_results <- get_metrics_indi_tr(data = eval(parse(text = paste0(cohort, "_rf_fit")))$pre$actions$recipe$recipe$template, 
                                      model = eval(parse(text = paste0(cohort, "_rf_fit"))), 
                                      level = "genus", labels = eval(parse(text = paste0(cohort, "_rf_fit")))$pre$mold$outcomes$labels, cohort = cohort)
  ...
  all_metrics <- list("tsementzi_rf" = get_metrics(data = combined, combined[, 1]))
...
tsementzi_microbiomecl_training <- get_results_microbiome_tsementzi1(cohort = "tsementzi",type = "training")
tsementzi_microbiomecl_testing <- get_results_microbiome_tsementzi1(cohort = "tsementzi",type = "testing")

walsh_microbiomecl_training <- get_results_microbiome_walsh(cohort = "walsh",type = "training")
walsh_microbiomecl_testing <- get_results_microbiome_walsh(cohort = "walsh",type = "testing")
...
all <- all %>% mutate(ensemble_pred = case_when((round(ensemble, 1)) >=0.5 ~ "EC",
                                                (round(ensemble, 1)) < 0.5 ~ "Benign"))
...
write.table(all, "~/Desktop/clinical_no_ph_micro.csv", quote = F, sep = ",", row.names = F)
