...
prep_test_age_bmi <- function(data, trained_data){
...
training_data_complete$labels <- factor(sample_data(Walsh_solo_res$raw)$histology, levels = c("Benign", "EC"))
...
rf_grid <- expand.grid(mtry = seq(5, 200, 1), min_n = seq(2, 12, 2), trees = 2000)
...
final_rf_wf <- rand_forest_wflow %>% 
  finalize_workflow(list(mtry = 79, trees = 2000, min_n = 2))
final_fit_rf <- final_rf_wf %>%
  fit(training_data_complete_smote)
#rf_results_chao_grid <- get_metrics_indi(data = test_data_complete, model = final_fit_rf, level = "genus", labels = data.frame(sample_data(Antonio1_solo_res$raw))$histology)
saveRDS(rand_forest_res_grid, "~/Desktop/thesis/VM02_meta_analysis/src/updated/updated1/Walsh_clinical_no_ph_micro_rf_grid.rds")
saveRDS(final_fit_rf, "~/Desktop/thesis/VM02_meta_analysis/src/updated/updated1/Walsh1_clinical_no_phmicro_rf_fit.rds")
...
saveRDS(nnet_res, "~/Desktop/thesis/VM02_meta_analysis/src/updated/updated1/Walsh_clinical_no_ph_micro_nnet_grid.rds")
saveRDS(final_fit_nnet, "~/Desktop/thesis/VM02_meta_analysis/src/updated/updated1/Walsh_clinical_no_ph_micro_nnet_fit.rds")
...
bt_spec <- boost_tree(mtry = tune(), trees = tune(), min_n = tune(), 
  ...
  set_mode("classification") %>%
  set_engine("xgboost")
xgb_rec <- recipe(labels ~ ., data = training_data_complete_smote) %>%
  step_normalize(age, shannon, bmi) %>%
...
xgb_grid <- expand.grid(mtry = seq(5, 150, 3), trees = 1500, min_n = seq(2, 10, 3), learn_rate = 0.3, 
...
final_bt_wf <- bt_wflow %>% 
  finalize_workflow(list(mtry = 29, trees = 1500, min_n = 2, tree_depth = 2, learn_rate = 0.3, 
                             loss_reduction = 0, sample_size = 1, stop_iter = Inf))
...
saveRDS(xgb_res, "~/Desktop/thesis/VM02_meta_analysis/src/updated/updated1/Walsh1_clinical_no_ph_micro_xgb_grid.rds")
saveRDS(final_fit_bt, "~/Desktop/thesis/VM02_meta_analysis/src/updated/updated1/Walsh1_clinical_no_ph_micro_xgb_fit.rds")

## Stack models 
stacks_model <- stacks() %>%
  add_candidates(candidates = rand_forest_res_grid, name = "rf") %>%
...
saveRDS(final_fit_bt, "~/Desktop/Chao_full_model.rds")
