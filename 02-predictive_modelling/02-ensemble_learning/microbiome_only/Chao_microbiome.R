...
training_data_smote <- themis::smote(training_data, var = "labels", k = neigh, over_ratio = 1)
set.seed(123)
...
rf_grid <- expand.grid(mtry = seq(15, 150, 10), min_n = seq(1, 15, 2), trees = seq(1500, 2500, 100))
...
saveRDS(rand_forest_res_grid, "~/Desktop/thesis/VM02_meta_analysis/src/updated/Chao_microbiome_rf_grid.rds")
saveRDS(final_fit_rf, "~/Desktop/thesis/VM02_meta_analysis/src/updated/Chao_microbiome_rf_fit.rds")
...
nnet_grid1 <- expand.grid(hidden_units = seq(2, 12, 1), penalty = seq(0, 1, 0.1), epochs = seq(5, 11, 2))

nnet_res <-tune_grid(object = nnet_wflow, metrics = metric, resamples = folds, 
                     grid = nnet_grid, control = ctrl_res)

nnet_res1 <-tune_grid(object = nnet_wflow, metrics = metric, resamples = folds, 
                     grid = nnet_grid1, control = ctrl_res)
set.seed(123)
...
saveRDS(nnet_res, "~/Desktop/thesis/VM02_meta_analysis/src/updated/Chao_microbiome_nnet_grid.rds")
saveRDS(final_fit_nnet, "~/Desktop/thesis/VM02_meta_analysis/src/updated/Chao_microbiome_nnet_fit.rds")
...
saveRDS(xgb_res, "~/Desktop/thesis/VM02_meta_analysis/src/updated/Chao_microbiome_xgb_grid.rds")
saveRDS(final_fit_bt, "~/Desktop/thesis/VM02_meta_analysis/src/updated/Chao_microbiome_xgb_fit.rds")
