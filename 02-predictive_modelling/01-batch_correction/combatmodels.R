...
combat_data <- phylo_obj_agg_combat
training_combat_data<- data.frame(otu_table(combat_data))
training_combat_data$labels <- factor(sample_data(combat_data)$histology, levels = c("Benign", "EC"))

shannon_train <- as.data.frame(microbiome::alpha(combat_data, 
                                                 index = c("diversity_shannon")))
training_combat_data$shannon <- (shannon_train$diversity_shannon)



neigh <- ifelse(min(table(training_combat_data$labels))>25, 5, min(table(training_combat_data$labels))-1)
training_combat_data$labels <- as.factor(training_combat_data$labels)
training_combat_data <- janitor::clean_names(training_combat_data)
training_combat_data_smote <- themis::smote(training_combat_data, var = "labels", k = neigh, over_ratio = 1)
set.seed(123)
folds <- rsample::vfold_cv(training_combat_data_smote, strata = "labels", v = 5, repeats = 3)
## remove correlation filter for base models 
data_rec <- recipe(labels ~ ., data = training_combat_data_smote) %>%
...
rf_grid <- expand.grid(mtry = seq(150, 300, 5), min_n = seq(1, 15, 2), trees = seq(1500, 2500, 100))
...
saveRDS(rand_forest_res_grid, "~/Desktop/thesis/VM01_EC_predictive_signature/02-predictive_modelling/01-batch_correction/combat_rf_grid.rds")
saveRDS(final_fit_rf, "~/Desktop/thesis/VM01_EC_predictive_signature/02-predictive_modelling/01-batch_correction/combat_rf_fit.rds")
...
saveRDS(nnet_res, "~/Desktop/thesis/VM01_EC_predictive_signature/02-predictive_modelling/01-batch_correction/combat_nnet_grid.rds")
saveRDS(final_fit_nnet, "~/Desktop/thesis/VM01_EC_predictive_signature/02-predictive_modelling/01-batch_correction/combat_nnet_fit.rds")
...
saveRDS(bt_res, "~/Desktop/thesis/VM01_EC_predictive_signature/02-predictive_modelling/01-batch_correction/combat_xgb_grid.rds")
saveRDS(final_fit_bt, "~/Desktop/thesis/VM01_EC_predictive_signature/02-predictive_modelling/01-batch_correction/combat_xgb_fit.rds")
